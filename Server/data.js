...
function registerTeacher(args, callback) {
    ...
    mongoh.Exists(db, "users", {
        ...
        if (result) {
            ...
            var password = '123456789';/*genPass.generate({
                length: 8,
                numbers: true
            });*/
    ...
    db.collection("groups").aggregate([
        ...
        db.collection("users").updateOne({
            [identifier]: userid
                ...
                $set: {
                    grades: result ? grades.sort((a, b) => { return b - a; }) : [],
...
function cleanUpGroupLinks(groupid, callback) {
    db.collection("groupslinks").updateMany({
        'links.groupid': groupid
            ...
            $pull: { links: { groupid: groupid } }
        }, () => { db.collection("groupslinks").deleteMany({ links: { $size: 0 } }, () => { }); })
    ...
    if (args.classnum == -1) {
        db.collection("groupslinks").updateMany({
            ...
            grade: args.grade
                ...
                $pull: { links: { day: args.groupday, groupid: args.groupid } }
            }, {}, (err) => {
                if (err) callback(err);
                else callback(null, stats.OK);
                db.collection("groupslinks").deleteMany({ links: { $size: 0 } }, () => { });
        ...
        db.collection("groupslinks").updateMany({
            ...
            grade: args.grade
                ...
                }, { $push: { links: { day: args.groupday, groupid: args.groupid } } }, { upsert: true }, (err) => {
                    if (err) callback(err);
                    else callback(null, stats.OK);
                });
            })
...
function listGroupClassesLinks(args, callback) {
    db.collection('groups').aggregate([
        ...
        db.collection("groupslinks").find({
            [teacherForeignIdentifier]: teacherRep(args.userDoc),
            grade: args.grade
        }, { _id: 0, [teacherForeignIdentifier]: 0 }).toArray((err, links) => {
            if (err) callback(err);
            else callback(null, stats.OK, { classescount: result[0] ? result[0].len : 0, links: links });
        });
    })
}
...
function editGroup(args, callback) {
    ...
    if (updateQuery.schedule.days) {
        db.collection("groups").findOne({
            id: args.id,
            [foreignIdentifier]: teacherRep(args.userDoc)
        }, { 'schedule.days': 1, grade: 1 }, (err, group) => {
            if (err) callback(err);
            else update(() => {
                ...
                if (diff.changed && diff.removed.length > 0) {
                    db.collection('groupslinks').updateMany({
                        ...
                        grade: group.grade
                            ...
                            $pull: {
                                links: { groupid: args.id, day: { $in: diff.removed } }
                            }
                        }, () => { db.collection("groupslinks").deleteMany({ links: { $size: 0 } }, () => { }); });
                }
            });
...
function removeGroupClass(args, callback) {
    db.collection('groupslinks').updateMany({
        ...
        grade: args.grade
            ...
            $pull: {
                links: { groupid: args.groupid, day: args.groupday }
            }
        }, (err) => {
            db.collection("groupslinks").deleteMany({ links: { $size: 0 } }, () => { });
            db.collection("groups").updateOne({
                id: args.groupid,
                [foreignIdentifier]: teacherRep(args.userDoc)
            }, { $pull: { 'schedule.days': args.groupday } }, (err) => {
                if (err) callback(err);
                else {
                    callback(null, stats.OK);
                }
...
function setDefaultStartDate(args, callback) {
    db.collection('users').updateOne({
        teacherid: teacherRep(args.userDoc)
    }, { $pull: { defaultStartDates: { grade: args.grade } } }, (err) => {
        if (err) return callback(err);
        db.collection('users').updateOne({
            teacherid: teacherRep(args.userDoc)
        }, { $push: { defaultStartDates: { grade: args.grade, date: new Date(args.date) } } }, (err) => {
            if (err) callback(err);
            callback(null, stats.OK);
        });
    });
...
function getDefaultStartDates(args, callback) {
    db.collection('users').findOne({
        teacherid: teacherRep(args.userDoc)
    }, { grades: 1, defaultStartDates: 1, _id: 0 }, (err, result) => {
        if (err) callback(err);
        else {

            callback(null, stats.OK, { grades: result.grades, startDates: result.defaultStartDates });
        }
    })
}
...
function logStudentsCount() {
    ...
    db.collection("links").aggregate([
        {
            $group: {
                _id: { teacherid: "$teacherid", grade: "$grade" }, count: { $sum: 1 }
            }
        }, {
            $project: {
                _id: 0, "teacherid": "$_id.teacherid", count: "$count", date: d, grade: "$_id.grade"
            }
        }], {}, (err, result) => {
            if (err) SaveError(err);
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    db.collection("counter").updateOne({ date: d }, result[i], { upsert: true }, (err) => {
                        if (err) SaveError(err);
                    })
                }
            }
...
function countGroupsLinks(args, callback) {
    db.collection("groupslinks").find({ teacherid: teacherRep(args.userDoc), grade: args.grade }).count((err, count) => {
        if (err) callback(err);
...
function listItems(args, callback) {
    db.collection("items").find({
        teacherid: teacherRep(args.userDoc),
        grade: args.grade
    }, { _id: 0 }).sort({ category: 1 }).toArray((err, result) => {
        if (err) callback(err);
        else callback(null, stats.OK, result);
    });
}
...
function requestParentToken(args, callback) {
    ...
    }, { _id: 0, [identifier]: 1, fullname: 1 }, (err, result) => {
        if (err) return callback(err);
        if (result) {
    ...
    db.collection("parentTokens").findOne({
        ...
        if (result) {
            db.collection("links").aggregate([
                ...
                if (err) callback(err);
                else callback(null, stats.OK, { fullname: result.fullname, links: links });
            })
...
function fetchLogsForParent(args, callback) {
    db.collection("parentTokens").findOne({
        token: args.parenttoken
    }, { _id: 0, [studentForeignIdentifier]: 1 }, (err, result) => {
        if (err) return callback(err);
        if (result) {
            var fetchArgs = {
                userDoc: {
...
function fetchLogs(args, callback) {
    ...
    if (args.datePeriod) {
        exam_query['$and'] = [
            { date: { $gte: args.datePeriod.start } },
            { date: { $lte: args.datePeriod.end } }
        ]
    }
    db.collection("exams").find(exam_query, {
        _id: 0
        // id for exams because date cannot be edited
    }).sort({ id: -1 }).toArray((err, exams) => {
        if (err) return callback(err);
        let class_query = {
            [teacherForeignIdentifier]: teacherRep(args.userDoc),
            grade: args.grade
        ...
        }).sort({ date: -1 }).toArray((err, classes) => {
            if (err) return callback(err);
            db.collection("logs").find({
                [teacherForeignIdentifier]: teacherRep(args.userDoc),
                [studentForeignIdentifier]: args.targetuser
                    ...
                    db.collection('items').find({
                        [teacherForeignIdentifier]: teacherRep(args.userDoc),
                        grade: args.grade
                    }, { _id: 0, name: 1, price: 1, id: 1 }).toArray((err, items) => {
                        if (err) return callback(err);
                        db.collection('payments').find({
                            [teacherForeignIdentifier]: teacherRep(args.userDoc),
                            [studentForeignIdentifier]: args.targetuser
                        }, { _id: 0, }).toArray((err, payments) => {
                            if (err) return callback(err);
                            let itemsids = {};
    ...
    db.collection("links").aggregate([
        ...
        db.collection("classes").aggregate([{
            ...
            db.collection("exams").aggregate([{
                ...
                if (err) callback(err);
                else callback(null, stats.OK, { data: data, classes: classes, exams: exams });
            })
        })
    });
}
