...
const host = loginStorage.getItem('host') || "127.0.0.1";
const port = 8080;
...
sdk.factory('sdk', ['$http', function ($http) {
  ...
  function CheckToken(student, teacher, def) {
    var token = loginStorage.getItem('token');
    if (token) {
  ...
  function CreateGrade(name, callback) {
    const token = loginStorage.getItem('token');
  ...
  var html = `
    ...
    <script>
      $(document).ready(() => { 
        setTimeout(() => {
          if (localStorage.getItem('testPrint')) {
            return;
          }
          window.print();
          window.close();
  ...
  function GenerateReceipt(data) {
    const barcodeHeight = Number(loginStorage.getItem('barcodeHeight'));
    const {
      id,
      fullname,
      profile,
