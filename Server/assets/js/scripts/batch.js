...
app.run(function ($rootScope, $window, $location, sdk) {
  ...
  barcodeScanner((code) =>
    $rootScope.barcodeObservers.forEach((observer) => observer(code))
  );
...
app.controller("mainCtrl", function ($rootScope, $scope, sdk) {
    ...
    switch ($scope.selected_timeSelector) {
      ...
      case "lastweek":
        $("#start_date").datepicker(
          "setDate",
          new Date(d.getTime() - 1000 * 60 * 60 * 24 * 7)
        );
  ...
  $scope.refetchLogs = (id, cb) => {
    ...
    if (new Date(sdate) != "Invalid Date" && new Date(edate) != "Invalid Date")
      datePeriod = {
        start: sdate,
        end: edate,
      };
    ...
    sdk.FetchLogs(id, $scope.selected_grade, datePeriod, (stat, result) => {
      ...
      if (stat == sdk.stats.OK) {
        ...
        report.examsAverage = Math.round(
          (examsMarksTotal / examsMaxTotal) * 100
        );
        ...
        for (const classLog of report.classes) {
          ...
          if (
            classLog.log &&
            classLog.log.homework &&
            classLog.log.homework.type == "marks"
          ) {
        ...
        report.quizAverage = Math.round((quizMarksTotal / quizMaxTotal) * 100);
        report.homeworkAverage = Math.round(
          (homeworkMarksTotal / homeworkMaxTotal) * 100
        );
        report.classAverage = Math.round(
          ((quizMarksTotal + homeworkMarksTotal) /
            (quizMaxTotal + homeworkMaxTotal)) *
            100
        );
        report.examQuizAverage = Math.round(
          ((quizMarksTotal + examsMarksTotal) /
            (quizMaxTotal + examsMaxTotal)) *
            100
        );
        report.totalAverage = Math.round(
          ((quizMarksTotal + homeworkMarksTotal + examsMarksTotal) /
            (quizMaxTotal + homeworkMaxTotal + examsMaxTotal)) *
            100
        );
        ...
        for (let i = 0; i < result.items.length; i++) {
          ...
          if (!payment.payed) {
            ...
            if (payedMoney > item.price) {
              ...
              result.items[i].payText =
                "دفع أكثر من سعرها و يتبقى له " +
                (payedMoney - item.price) +
                " جنيه";
            } else if (payedMoney < item.price) {
              result.items[i].payClass = "purple-text";
              result.items[i].payText =
                "لم يدفع بالكامل متبقى " + (item.price - payedMoney) + " جنيه";
            } else if (payedMoney == item.price) {
