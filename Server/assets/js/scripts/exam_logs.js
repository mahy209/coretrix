...
app.controller("mainCtrl", function ($rootScope, $scope, sdk) {
  ...
  $scope.blurTable = Cookies.get("blurLogsTable")
    ? Cookies.get("blurLogsTable") == "true"
    : true;
  $scope.setBlurCookie = () => {
  ...
  $scope.sendSMS = () => {
    window.open(
      `/cpanel#tab=sms&type=exam&grade=${$scope.selected_grade}&exam=${$scope.selected_exam.id}`,
      "_blank"
    );
  };
        ...
        case sdk.stats.OK:
          ...
          if (result.indexOf(grade) > -1) {
            ...
            $scope.grade_changed(() => {
              ...
              if (!found)
                toast("لا توجد سجلات بهذا الإمتحان!", gradients.error);
            });
          } else {
  ...
  $scope.addExam = () => {
    if ($scope.newExam_name == null)
      return toast("من فضلك قم بإدخال اسم الإمتحان", gradients.error);
    if ($scope.addExam_selectedGrade == null)
      return toast("من فضلك قم بإختيار السنة", gradients.error);
    sdk.AddExam(
      $scope.newExam_name,
      parseInt($scope.addExam_selectedGrade),
      $scope.newExam_max,
      (stat) => {
        switch (stat) {
          case sdk.stats.OK:
  ...
  $scope.reload = () => {
    if (isNaN($scope.selected_grade) && $scope.selected_grade != "all")
      return toast("لم يتم إختيار السنة!", gradients.error);
    ...
    sdk.FetchExamLogs(
      $scope.selected_exam.id,
      $scope.selected_exam.grade,
      (stat, logs) => {
  ...
  $scope.editExam = () => {
    if (!$scope.editExam_name)
      return toast("برجاء كتابة اسم الإمتحان!", gradients.error);
    if (!$scope.editExam_max)
      return toast("برجاء إدخال أعلى درجة للإمتحان!", gradients.error);
    if (!($scope.editExam_redline >= 0))
      return toast("برجاء إدخال الخط الأحمر للدرجات!", gradients.error);
    sdk.EditExam(
      $scope.loadedExamId,
      $scope.editExam_name,
      $scope.editExam_max,
      $scope.editExam_redline,
      (stat) => {
        if (stat == sdk.stats.OK) {
          $scope.grade_changed();
  ...
  $scope.grade_changed = (callback) => {
    if (!isNaN($scope.selected_grade) || $scope.selected_grade == "all") {
      sdk.ListExams(
        $scope.selected_grade == "all"
          ? undefined
          : parseInt($scope.selected_grade),
        (stat, exams) => {
          switch (stat) {
            case sdk.stats.OK:
              const examid = $scope.selected_exam
                ? $scope.selected_exam.id
                : undefined;
              $scope.exams = exams;
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (cells.length > 1) {
          ...
          $scope.$apply(function () {
            if (entity) {
              entity.log.mark = score;
              $scope.markChanged(entity);
            }
          });
        }
      }
